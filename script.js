...
function makeCard(item) {
  ...
  if (item.meta) {
    const mWrap = el("div", "card__meta");
    if (item.meta.award) { const m = el("div", "m-item"); m.innerHTML = `🏅 <span>${item.meta.award}</span>`; mWrap.appendChild(m); }
    if (item.meta.org) { const m = el("div", "m-item"); m.innerHTML = `🏛️ <span>${item.meta.org}</span>`; mWrap.appendChild(m); }
    if (item.meta.date) { const m = el("div", "m-item"); m.innerHTML = `📅 <span>${item.meta.date}</span>`; mWrap.appendChild(m); }
